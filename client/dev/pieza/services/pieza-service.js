"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var core_1 = require('angular2/core');
var http_1 = require('angular2/http');
require('rxjs/add/operator/map');
var Pieza = (function () {
    function Pieza(_id, modelo, estado, lote, caracterisiticas, almacen, almacenOrigen, vendido, compuestoPor, precio) {
        this._id = _id;
        this.modelo = modelo;
        this.estado = estado;
        this.lote = lote;
        this.caracterisiticas = caracterisiticas;
        this.almacen = almacen;
        this.almacenOrigen = almacenOrigen;
        this.vendido = vendido;
        this.compuestoPor = compuestoPor;
        this.precio = precio;
    }
    return Pieza;
}());
exports.Pieza = Pieza;
var PiezaService = (function () {
    function PiezaService(_http) {
        this._http = _http;
    }
    PiezaService.prototype.getAll = function () {
        return this._http
            .get(PiezaService.ENDPOINT.replace(':id', ''))
            .map(function (r) { return r.json(); });
    };
    PiezaService.prototype.gotoIndex = function () {
        return this._http
            .get(PiezaService.ENDPOINT.replace(':id', ''))
            .map(function (r) { return r.json(); });
    };
    PiezaService.prototype.getPiezaId = function (id) {
        return this._http
            .get(PiezaService.ENDPOINT.replace(':id', id))
            .map(function (r) { return r.json(); });
    };
    PiezaService.prototype.getPiezaName = function (nombre) {
        //alert("al service le llega " + nombre);
        return this._http
            .get(PiezaService.ENDPOINT2.replace(':nombre', nombre))
            .map(function (r) { return r.json(); });
    };
    PiezaService.prototype.add = function (_id, modelo, estado, lote, caracteristicas, almacen, almacenOrigen, vendido, compuestoPor, precio) {
        var body = JSON.stringify({ _id: _id, modelo: modelo, estado: estado, lote: lote, caracteristicas: caracteristicas, almacen: almacen, almacenOrigen: almacenOrigen, vendido: vendido, compuestoPor: compuestoPor, precio: precio });
        alert("body" + body);
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        return this._http
            .post(PiezaService.ENDPOINT.replace(':id', ''), body, { headers: headers })
            .map(function (r) { return r.json(); });
    };
    PiezaService.prototype.remove = function (id) {
        return this._http
            .delete(PiezaService.ENDPOINT.replace(':id', id));
    };
    PiezaService.ENDPOINT = '/api/pieza/:id';
    PiezaService.ENDPOINT2 = '/api/pieza/details/:nombre';
    PiezaService = __decorate([
        __param(0, core_1.Inject(http_1.Http)), 
        __metadata('design:paramtypes', [http_1.Http])
    ], PiezaService);
    return PiezaService;
}());
exports.PiezaService = PiezaService;
